# PowerTree-Fixed.ps1
# This is a fixed version of PowerTree.ps1 that handles spaces between numbers and units
# and properly converts file sizes in PowerShell 5+
...
function PowerTree {
    ...
    param(
        ...
        [ValidateScript({
            ...
            # Allow spaces between number and unit
            $_ -match '^\-?\d+(?:\.\d+)?\s*(b|kb|mb|gb|tb)?$'
        })]
        [Alias('fsmi')]
